...
func socketPair(t *testing.T) (*websocket.Conn, *websocket.Conn) {
	...
	server := <-accepted
	t.Cleanup(func() { _ = server.Close(); _ = ws.Close(); close(done); s.Close() })
	return server, ws
}
...
func TestSlowWriterDoesNotDelayOtherClient(t *testing.T) {
	...
	fast := New(fastServer)
	defer fast.Close()
	if err := fast.SendBytes([]byte(`{"ok":true}`)); err != nil {
		t.Fatal(err)
	}
	_ = fastReader.SetReadDeadline(time.Now().Add(time.Second))
	var raw string
...
func TestFinishPreservesOrderThenCloses(t *testing.T) {
	...
	c := New(s)
	defer c.Close()
	if err := c.SendBytes([]byte(`{"first":1}`)); err != nil {
		t.Fatal(err)
	}
	if err := c.Finish(map[string]int{"last": 2}); err != nil {
		t.Fatal(err)
	}
